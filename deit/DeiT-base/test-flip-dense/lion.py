    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                #         decay = 0.006
                #         p.grad.add_(p.data * (1 - p.mask), alpha=decay)
                if getattr(p, 'old_mask', None) is not None:
                    assert getattr(p, 'mask') is not None
                    flips.append(
                        torch.zeros_like(p).bool() if (getattr(p, 'old_mask')).all() else getattr(p, 'mask') != getattr(
                            p, 'old_mask'))
                    p.cnt = 0
        ...
        return loss, torch.sum(flips), torch.mean(flips.float())
