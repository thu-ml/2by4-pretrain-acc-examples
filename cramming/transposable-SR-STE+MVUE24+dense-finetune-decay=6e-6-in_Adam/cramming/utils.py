...
def main_launcher(cfg, main_fn, job_name=""):
    ...
    with open_dict(cfg):
        cfg.original_cwd = hydra.utils.get_original_cwd()  # /mnt/e/cramming
        ...
        if not os.path.isabs(cfg.base_dir):
            ...
            cfg.base_dir = base_dir_full_path  # /mnt/e/cramming/outputs

        cfg.impl.path = os.path.expanduser(cfg.impl.path)  # data
    ...
    log.info("-------------------------------------------------------------")
    log.info(
        f"Finished running job {cfg.name} with total train time: " f"{str(datetime.timedelta(seconds=time.time() - launch_time))}")
...
def find_pretrained_checkpoint(cfg, downstream_classes=None):
    ...
    if checkpoint_name is not None:
        ...
        if cfg.eval.arch_modifications is not None and cfg.eval.arch_modifications != 'None':
...
def dump_metrics(cfg, metrics):
    ...
    try:
        sanitized_metrics['cola']['matthews_correlation'] = float(sanitized_metrics['cola']['matthews_correlation'])
    except:
        pass
