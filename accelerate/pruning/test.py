...
from sparse.legacy import transposable_sparse, sparse24
...
def benchmark(size, provider):
    N = 64
    n = 2048
    x = torch.rand(size, N * n, device='cuda', dtype=torch.half).t()
    quantiles = [0.5, 0.2, 0.8]
    if provider == 'torch':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: F.relu(x), quantiles=quantiles)
    if provider == 'triton':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: to_sparse_semi_structured(x, MVUE24=True),
                                                     quantiles=quantiles)
