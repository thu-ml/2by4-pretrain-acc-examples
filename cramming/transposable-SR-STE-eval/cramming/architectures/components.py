...
from sparse.GELUglu import GELUglu
...
class FFNComponent(torch.nn.Module):
    ...
    def __init__(self, hidden_size, intermed_size, nonlin_fn=torch.nn.GELU, use_bias=True):
        super().__init__()
        self.dense_in = SparseLinearTranspose(hidden_size, intermed_size, bias=use_bias,
                                              func=lambda step: 'sparse' if step <= 250000 else 'dense'
                                              )
        ...
        self.nonlin = GELUglu()
        intermed_output_size = intermed_size // 2
        self.dense_out = SparseLinearTranspose(intermed_output_size, hidden_size, bias=use_bias,
                                               func=lambda step: 'sparse' if step <= 250000 else 'dense'
                                               )
...
class TransformerLayer(torch.nn.Module):
    ...
    def __init__(self, idx, cfg_arch):
        ...
        if self.norm_scheme == "sandwich":
            ...
            self.fn = get_layer_fn(
                type="sandwich", scripting=cfg_arch.layer_fusion, dn=cfg_arch.deepnorm_scaling,
    ...
    def forward(self, states, attention_mask: Optional[torch.Tensor] = None, res_scale=1):
        if self.norm_scheme == "pre":  # On Layer Normalization in the Transformer Architecture
            ...
            states = self.fn(states, self.norm3(self.dropout(self.attn(self.norm1(states), attention_mask))),
                             self.alpha, res_scale)
...
class PoolingComponent(torch.nn.Module):
    def __init__(self, cfg_head, main_model_hidden_size):
        super().__init__()
        self.dense = torch.nn.Linear(main_model_hidden_size,
                                     cfg_head.head_dim) if cfg_head.include_ff_layer else torch.nn.Identity()
...
def get_extended_attention_mask(attention_mask: torch.Tensor, input_shape: Tuple[int],
                                causal_attention: bool = False) -> torch.Tensor:
    """
        ...
        if causal_attention:
            ...
            if causal_mask.shape[1] < attention_mask.shape[1]:
                ...
                causal_mask = torch.cat(
                    [
                        torch.ones((batch_size, seq_length, prefix_seq_len), device=attention_mask.device,
