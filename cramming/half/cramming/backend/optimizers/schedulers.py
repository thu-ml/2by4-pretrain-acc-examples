...
def get_schedule_fn(cfg_train):
    ...
    elif cfg_train.scheduler == "progress-triangle":
        scheduler_fn = get_progress_triangle
...
def get_budget_linear_schedule_with_warmup(optimizer, hour_budget, num_warmup_steps, num_training_steps,
                                           num_cycles=0.5):
...
def get_budget_cosine_schedule_with_warmup(optimizer, hour_budget, num_warmup_steps, num_training_steps,
                                           num_cycles=0.5):
    """Follows the huggingface transformers scheduler with the same name, but gets an additional arg hour_budget"""
    initial_time = time.time()
...
def get_budget_cosine_half_cycles_with_warmup(optimizer, hour_budget, num_warmup_steps, num_training_steps,
                                              num_cycles=0.5):
    """Follows the huggingface transformers scheduler with the same name, but gets an additional arg hour_budget"""
    initial_time = time.time()
...
def get_budget_inv_cosine_schedule(optimizer, hour_budget, num_cooldown_steps, num_training_steps, num_cycles=0.5):
    ...
    max_lr = max(0.0, 0.5 * (
            1.0 + math.cos(math.pi * float(num_cycles) * 2.0 * (1 - ult_step / float(max(1, num_training_steps))))))
...
def get_progress_triangle(optimizer):
    def lr_lambda(current_step):  # e-3
        x = current_step / 300000
        return min(0.25 + x, 4 - 4 * x)
...
def get_budget_polynomial_decay_with_warmup(optimizer, hour_budget, num_warmup_steps, num_training_steps, lr_end=0.0,
                                            power=1.0):
    """Follows the huggingface transformers scheduler with the same name, but gets an additional arg hour_budget"""
    initial_time = time.time()
