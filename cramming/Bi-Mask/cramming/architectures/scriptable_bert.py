...
class ScriptableLM(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.cfg.recurrent_layers is None:  # here comes all FNNs, torch.nn.ModuleList makes sure all FNNs parameters are added in the ScriptableLM().parameters()
            self.layers = torch.nn.ModuleList(
                [layer_fn(idx, self.cfg) for idx in range(self.cfg.num_transformer_layers)])
        else:
    ...
    def __init__(self, config):
        ...
        if self.cfg.loss == "szegedy":
            ...
            if self.cfg.tie_weights:
                self.decoder = torch.nn.Linear(self.cfg.embedding.embedding_dim, self.cfg.embedding.vocab_size,
                                               bias=self.cfg.decoder_bias)
                self.decoder.weight = self.encoder.embedding.word_embedding.weight
            else:
                self.decoder = torch.nn.Linear(self.cfg.hidden_size, self.cfg.embedding.vocab_size,
                                               bias=self.cfg.decoder_bias)

        self.loss_fn = _get_loss_fn(self.cfg.loss, z_loss_factor=self.cfg.z_loss_factor,
