...
import cramming
from cramming.architectures.components import FFNComponent
from cramming.backend.torch_default import TorchEngine
...
def main_training_process(cfg, setup):
    ...
    for step, batch in iterable_data:
        ...
        if cfg.train.scheduler == "progress-triangle":
            if step * cfg.impl.microbatch_size == 300000 * 128:
    ...
    if cramming.utils.is_main_process():
        ...
        model_engine.save_final_model(os.path.join(cfg.base_dir, cfg.name), checkpoint_id, tokenizer, cfg.arch,
                                      cfg.dryrun)
