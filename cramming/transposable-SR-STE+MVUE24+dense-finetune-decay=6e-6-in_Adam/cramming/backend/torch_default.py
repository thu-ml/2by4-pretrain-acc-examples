...
from .utils import group_parameters, prepare_pretraining_dataloader, torchdynamo_compile_method, update_ema, \
...
class TorchEngine(torch.nn.Module):
    ...
    def __init__(self, model, cfg_train, cfg_impl, setup=_default_setup, seq_length=128):
        ...
        amp_dtype = getattr(torch, self.cfg_impl.mixed_precision_target_dtype) if setup[
                                                                                      "device"].type != "cpu" else torch.bfloat16
        ...
        self.current_seq_length = seq_length if not self.sequence_curriculum else cfg_train.sequence_curriculum.lengths[
            0]
            ...
            with torch.no_grad():
                for n, p in list(self.model.named_parameters()):
                    if hasattr(p, 'mask') and p.mode == 'sparse':
                        p.grad = p.grad.float()
                        masked_add_(p.grad.data, p.data, p.mask, alpha=6e-6)
                        p.cnt = 0
        ...
        if not self.sequence_curriculum:
            ...
            if self.sequence_unfold:
                ...
                return self.current_seq_length * (
                        self.data_seq_length // self.current_seq_length) * self.cfg_impl.microbatch_size
            else:
    ...
    def moving_average_computation(self):
        if self.weight_averaging_frequency > 0:
            if (self.steps % self.weight_averaging_frequency) == 0:
                ...
                if self.weight_averaging.type == "EMA":
                    update_ema(params, self.param_store, buffers, self.buffer_store,
                               momentum=self.weight_averaging.momentum)
    ...
    def gradinit(self, data_iterable, optim_cfg, gradinit_cfg):
        ...
        adam_fn = partial(torch.optim._functional.adam, amsgrad=False, beta1=0.9, beta2=0.98, weight_decay=0, eps=1e-6,
                          maximize=False)
