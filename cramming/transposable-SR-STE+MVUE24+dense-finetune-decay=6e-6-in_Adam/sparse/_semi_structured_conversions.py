...
def _sparse_semi_structured_from_dense_bf16_kernel(
    ...
    x10, x20, x30, x40, x50, x60 = tl.abs(m00).to(tl.float32) > tl.abs(m10).to(tl.float32), tl.abs(m00).to(
        tl.float32) > tl.abs(m20).to(tl.float32), tl.abs(m00).to(tl.float32) > tl.abs(m30).to(tl.float32), tl.abs(
        m10).to(tl.float32) > tl.abs(m20).to(tl.float32), tl.abs(m10).to(tl.float32) > tl.abs(m30).to(
        tl.float32), tl.abs(m20).to(tl.float32) > tl.abs(m30).to(tl.float32)
    m00_, m10_, m20_, m30_ = x20 & x30 | x10 & x20 | x10 & x30, ~x10 & x50 | x40 & x50 | ~x10 & x40, ~x20 & ~x40 | ~x20 & x60 | ~x40 & x60, ~x30 & ~x50 | ~x30 & ~x60 | ~x50 & ~x60

    x11, x21, x31, x41, x51, x61 = tl.abs(m01).to(tl.float32) > tl.abs(m11).to(tl.float32), tl.abs(m01).to(
        tl.float32) > tl.abs(m21).to(tl.float32), tl.abs(m01).to(tl.float32) > tl.abs(m31).to(tl.float32), tl.abs(
        m11).to(tl.float32) > tl.abs(m21).to(tl.float32), tl.abs(m11).to(tl.float32) > tl.abs(m31).to(
        tl.float32), tl.abs(m21).to(tl.float32) > tl.abs(m31).to(tl.float32)
    m01_, m11_, m21_, m31_ = x21 & x31 | x11 & x21 | x11 & x31, ~x11 & x51 | x41 & x51 | ~x11 & x41, ~x21 & ~x41 | ~x21 & x61 | ~x41 & x61, ~x31 & ~x51 | ~x31 & ~x61 | ~x51 & ~x61

    x12, x22, x32, x42, x52, x62 = tl.abs(m02).to(tl.float32) > tl.abs(m12).to(tl.float32), tl.abs(m02).to(
        tl.float32) > tl.abs(m22).to(tl.float32), tl.abs(m02).to(tl.float32) > tl.abs(m32).to(tl.float32), tl.abs(
        m12).to(tl.float32) > tl.abs(m22).to(tl.float32), tl.abs(m12).to(tl.float32) > tl.abs(m32).to(
        tl.float32), tl.abs(m22).to(tl.float32) > tl.abs(m32).to(tl.float32)
    m02_, m12_, m22_, m32_ = x22 & x32 | x12 & x22 | x12 & x32, ~x12 & x52 | x42 & x52 | ~x12 & x42, ~x22 & ~x42 | ~x22 & x62 | ~x42 & x62, ~x32 & ~x52 | ~x32 & ~x62 | ~x52 & ~x62

    x13, x23, x33, x43, x53, x63 = tl.abs(m03).to(tl.float32) > tl.abs(m13).to(tl.float32), tl.abs(m03).to(
        tl.float32) > tl.abs(m23).to(tl.float32), tl.abs(m03).to(tl.float32) > tl.abs(m33).to(tl.float32), tl.abs(
        m13).to(tl.float32) > tl.abs(m23).to(tl.float32), tl.abs(m13).to(tl.float32) > tl.abs(m33).to(
        tl.float32), tl.abs(m23).to(tl.float32) > tl.abs(m33).to(tl.float32)
    m03_, m13_, m23_, m33_ = x23 & x33 | x13 & x23 | x13 & x33, ~x13 & x53 | x43 & x53 | ~x13 & x43, ~x23 & ~x43 | ~x23 & x63 | ~x43 & x63, ~x33 & ~x53 | ~x33 & ~x63 | ~x53 & ~x63
...
def _sparse_semi_structured_from_dense_triton(dense, sparse, meta, mask: Optional[Tensor] = None,
    ...
    if dtype not in [torch.float16, torch.bfloat16]:
        raise RuntimeError(f"Invalid datatype {dense.dtype} of dense matrix")
    # if dtype is torch.bfloat16 and mask is None:
    #     raise RuntimeError(f"Invalid datatype {dense.dtype} of dense matrix without a given mask")
    if m % 32 != 0:
        raise RuntimeError(
            f"Number rows columns of dense matrix {m} must be divisible by 32"
        )
...
def _sparse_semi_structured_from_dense_MVUE12_kernel(
    ...
    left0 = tl.abs(m00) + 1.19209e-07
    left1 = tl.abs(m20) + 1.19209e-07
    left2 = tl.abs(m01) + 1.19209e-07
    left3 = tl.abs(m21) + 1.19209e-07
    left4 = tl.abs(m02) + 1.19209e-07
    left5 = tl.abs(m22) + 1.19209e-07
    left6 = tl.abs(m03) + 1.19209e-07
    left7 = tl.abs(m23) + 1.19209e-07
    right0 = tl.abs(m10) + 1.19209e-07
    right1 = tl.abs(m30) + 1.19209e-07
    right2 = tl.abs(m11) + 1.19209e-07
    right3 = tl.abs(m31) + 1.19209e-07
    right4 = tl.abs(m12) + 1.19209e-07
    right5 = tl.abs(m32) + 1.19209e-07
    right6 = tl.abs(m13) + 1.19209e-07
    right7 = tl.abs(m33) + 1.19209e-07
...
def MVUE12(x1, x2,
           seed,
           BLOCK_SIZE: tl.constexpr):
    left = tl.abs(x1) + 1.19209e-07
    right = tl.abs(x2) + 1.19209e-07
    m = left + right
    p = left / m
    random = tl.rand(seed, tl.arange(0, BLOCK_SIZE))
    m1 = (random <= p)
    m2 = (random > p)
    left_ = tl.where(m1, x1, 0.0)
    right_ = tl.where(m2, x2, 0.0)
    a1 = left_ / left * m
    a2 = right_ / right * m
    return a1, a2, m1, m2
...
@triton.jit
def MVUE24_fake(x0, x1, x2, x3,
                seed0, seed1,
                BLOCK_SIZE: tl.constexpr):
    # case 0
    a00, a10, m00, m10 = MVUE12(x0, x1, seed0, BLOCK_SIZE)
    a20, a30, m20, m30 = MVUE12(x2, x3, seed1, BLOCK_SIZE)
    mux0 = ((x0 >= x2) & (x0 >= x3) & (x1 <= x2) & (x1 <= x3)) | ((x0 <= x2) & (x0 <= x3) & (x1 >= x2) & (x1 >= x3)) | (
            (x2 >= x0) & (x2 >= x1) & (x3 <= x0) & (x3 <= x1))

    # case 1
    a01, a21, m01, m21 = MVUE12(x0, x2, seed0, BLOCK_SIZE)
    a11, a31, m11, m31 = MVUE12(x1, x3, seed1, BLOCK_SIZE)
    mux1 = ((x0 >= x1) & (x0 >= x3) & (x2 <= x1) & (x2 <= x3)) | ((x0 <= x1) & (x0 <= x3) & (x2 >= x1) & (x2 >= x3)) | (
            (x1 >= x0) & (x1 >= x2) & (x3 <= x0) & (x3 <= x2)) | ((x1 <= x0) & (x1 <= x2) & (x3 >= x0) & (x3 >= x2))

    # case 2
    a02, a32, m02, m32 = MVUE12(x0, x3, seed0, BLOCK_SIZE)
    a12, a22, m12, m22 = MVUE12(x1, x2, seed1, BLOCK_SIZE)
    # mux2 = ((x0 >= x1) & (x0 >= x2) & (x3 <= x1) & (x3 <= x2)) | ((x0 <= x1) & (x0 <= x2) & (x3 >= x1) & (x3 >= x2)) | (
    #         (x1 >= x0) & (x1 >= x2) & (x2 <= x0) & (x2 <= x3)) | ((x1 <= x0) & (x1 <= x2) & (x2 >= x0) & (x2 >= x3))

    a0 = tl.where(mux0, a00, tl.where(mux1, a01, a02))
    a1 = tl.where(mux0, a10, tl.where(mux1, a11, a12))
    a2 = tl.where(mux0, a20, tl.where(mux1, a21, a22))
    a3 = tl.where(mux0, a30, tl.where(mux1, a31, a32))
    m0 = tl.where(mux0, m00, tl.where(mux1, m01, m02))
    m1 = tl.where(mux0, m10, tl.where(mux1, m11, m12))
    m2 = tl.where(mux0, m20, tl.where(mux1, m21, m22))
    m3 = tl.where(mux0, m30, tl.where(mux1, m31, m32))
    return a0, a1, a2, a3, m0, m1, m2, m3


@triton.jit
def MVUE24_approx(x0, x1, x2, x3,
                  seed0, seed1,
                  BLOCK_SIZE: tl.constexpr):
    # return MVUE24_fake(x0, x1, x2, x3, seed0, seed1, BLOCK_SIZE)
    eps = 1.19209e-07
    a0 = tl.abs(x0)
    a1 = tl.abs(x1)
    a2 = tl.abs(x2)
    ...
    random0 = tl.rand(seed0, tl.arange(0, BLOCK_SIZE))
    m0 = (random0 <= a0 / (sum + eps))
    m1 = ((random0 <= (a0 + a1) / (sum + eps)) & (random0 > a0 / (sum + eps)))
    m2 = ((random0 <= (a0 + a1 + a2) / (sum + eps)) & (random0 > (a0 + a1) / (sum + eps)))
    m3 = (random0 > (a0 + a1 + a2) / (sum + eps))

    d_a0 = tl.where(m0, 0.0, a0)
    d_a1 = tl.where(m1, 0.0, a1)
    d_a2 = tl.where(m2, 0.0, a2)
    d_a3 = tl.where(m3, 0.0, a3)
    d_sum = d_a0 + d_a1 + d_a2 + d_a3
    random1 = tl.rand(seed1, tl.arange(0, BLOCK_SIZE))
    d_m0 = (random1 <= d_a0 / (d_sum + eps))
    d_m1 = ((random1 <= (d_a0 + d_a1) / (d_sum + eps)) & (random1 > d_a0 / (d_sum + eps)))
    d_m2 = ((random1 <= (d_a0 + d_a1 + d_a2) / (d_sum + eps)) & (random1 > (d_a0 + d_a1) / (d_sum + eps)))
    d_m3 = (random1 > (d_a0 + d_a1 + d_a2) / (d_sum + eps))

    p0 = (a0 / (sum + eps) + (a1 / (sum + eps)) * (a0 / ((sum + eps) - a1)) + (a2 / (sum + eps)) * (
            a0 / ((sum + eps) - a2)) + (a3 / (sum + eps)) * (a0 / ((sum + eps) - a3)))
    p1 = (a1 / (sum + eps) + (a0 / (sum + eps)) * (a1 / ((sum + eps) - a0)) + (a2 / (sum + eps)) * (
            a1 / ((sum + eps) - a2)) + (a3 / (sum + eps)) * (a1 / ((sum + eps) - a3)))
    p2 = (a2 / (sum + eps) + (a0 / (sum + eps)) * (a2 / ((sum + eps) - a0)) + (a1 / (sum + eps)) * (
            a2 / ((sum + eps) - a1)) + (a3 / (sum + eps)) * (a2 / ((sum + eps) - a3)))
    p3 = (a3 / (sum + eps) + (a0 / (sum + eps)) * (a3 / ((sum + eps) - a0)) + (a1 / (sum + eps)) * (
            a3 / ((sum + eps) - a1)) + (a2 / (sum + eps)) * (a3 / ((sum + eps) - a2)))

    m0, m1, m2, m3 = m0 | d_m0, m1 | d_m1, m2 | d_m2, m3 | d_m3
    a0 = tl.where(m0, x0 / (p0 + eps), 0.0)
    a1 = tl.where(m1, x1 / (p1 + eps), 0.0)
    a2 = tl.where(m2, x2 / (p2 + eps), 0.0)
    a3 = tl.where(m3, x3 / (p3 + eps), 0.0)

    return a0, a1, a2, a3, m0, m1, m2, m3


@triton.jit
def _sparse_semi_structured_from_dense_MVUE24_fake_kernel(
        dense_ptr,
        sparse_ptr,
        meta_reordered_ptr,
        dense_row_stride,
    ...
    m00, m10, m20, m30, m00_, m10_, m20_, m30_ = MVUE24_fake(m00, m10, m20, m30, seed0, seed1, BLOCK_SIZE // 16)
    m01, m11, m21, m31, m01_, m11_, m21_, m31_ = MVUE24_fake(m01, m11, m21, m31, seed2, seed3, BLOCK_SIZE // 16)
    m02, m12, m22, m32, m02_, m12_, m22_, m32_ = MVUE24_fake(m02, m12, m22, m32, seed4, seed5, BLOCK_SIZE // 16)
    m03, m13, m23, m33, m03_, m13_, m23_, m33_ = MVUE24_fake(m03, m13, m23, m33, seed6, seed7, BLOCK_SIZE // 16)
...
@triton.jit
def _sparse_semi_structured_from_dense_MVUE24_approx_kernel(
        dense_ptr,
        sparse_ptr,
        meta_reordered_ptr,
        dense_row_stride,
    ...
    m00, m10, m20, m30, m00_, m10_, m20_, m30_ = MVUE24_approx(m00, m10, m20, m30, seed0, seed1, BLOCK_SIZE // 16)
    m01, m11, m21, m31, m01_, m11_, m21_, m31_ = MVUE24_approx(m01, m11, m21, m31, seed2, seed3, BLOCK_SIZE // 16)
    m02, m12, m22, m32, m02_, m12_, m22_, m32_ = MVUE24_approx(m02, m12, m22, m32, seed4, seed5, BLOCK_SIZE // 16)
    m03, m13, m23, m33, m03_, m13_, m23_, m33_ = MVUE24_approx(m03, m13, m23, m33, seed6, seed7, BLOCK_SIZE // 16)
...
def _sparse_semi_structured_from_dense_triton_MVUE24_fake(dense, sparse, meta):
    if dense.dim() != 2:
        raise RuntimeError(
            f"Expected 2-dimensional dense tensor, got {dense.dim()}-dimensional tensor"
        )
    ...
    #     torch.empty(m, k // 16, device=device, dtype=torch.int16)
    seeds = torch.randint(0, 2 ** 31 - 1, (m, 8), device='cuda')
    _sparse_semi_structured_from_dense_MVUE24_fake_kernel[(m,)](
        dense,
        sparse,
        meta,
        dense.stride(0),
...
def _sparse_semi_structured_from_dense_triton_MVUE24_approx(dense, sparse, meta):
    if dense.dim() != 2:
        raise RuntimeError(
            f"Expected 2-dimensional dense tensor, got {dense.dim()}-dimensional tensor"
        )
    ...
    #     torch.empty(m, k // 16, device=device, dtype=torch.int16)
    seeds = torch.randint(0, 2 ** 31 - 1, (m, 8), device='cuda')
    _sparse_semi_structured_from_dense_MVUE24_approx_kernel[(m,)](
        dense,
        sparse,
        meta,
        dense.stride(0),
